...
with torch.no_grad():
    ...
    for i, batch in enumerate(test_loader):
        ...
        aspect_logits = outputs["aspect_logits"]  # (B,4)
        sentiment_logits = outputs["sentiment_logits"]  # (B,4,3)

        # =========================
        # Joint probability
        # =========================
        aspect_probs = torch.sigmoid(aspect_logits)  # (B,4)
        sentiment_probs = torch.softmax(sentiment_logits, dim=-1)  # (B,4,3)

        joint = aspect_probs.unsqueeze(-1) * sentiment_probs  # (B,4,3)
        ...
        # Flatten + top-2
        # =========================
        joint_flat = joint.view(B, -1)  # (B,12)

        topk_vals, topk_idx = torch.topk(joint_flat, k=2)

        # =========================
        # Convert to (aspect, sentiment)
        # =========================
        aspects = topk_idx // 3
        sentiments = topk_idx % 3

        # =========================
        # Build final_preds (same format as before)
        # =========================
        final_preds = torch.full((B, 4), 3, device=DEVICE)
        ...
        # =========================
        # Ground truth mapping (NEW)
        # =========================
        aspect_labels = batch["aspect_labels"]  # (B,4)
        sentiment_labels = batch["sentiment_labels"]  # (B,4)

        final_labels = torch.full((B, 4), 3, device=DEVICE)

        mask = aspect_labels == 1
        final_labels[mask] = sentiment_labels[mask]
